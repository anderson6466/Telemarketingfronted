  ...
  methods: {
    ...
    initialize () {
      this.$store.dispatch('pagoWebInit', null)
        .then((response) => {
          const formulario = localStorage.getItem('ap26499_formulario')
          if (formulario * 1 > 0) {
        ...
        this.$store.dispatch('pagoWebLinkDecrypt', request)
          .then((response) => {
            if (response.code === '01') {
              ...
              localStorage.setItem('ap26499_formulario', response.obj.afiliacion)
              this.beforeInitialize()
              this.$router.push('/zonasegura/pago2?producto=seguroVida')
            } else {
              this.error = true
              this.validador = false
              if (response.code === '98') {
      ...
      if (this.$refs.formPrimeraPrima.validate()) {
        ...
        this.$store.dispatch('pagoWebAction', request0)
          .then((response) => {
            ...
            } else if (response.code === '95') {
              this.showErrorApp = true
              this.errorMessageApp = process.env.error_pago_web_culqi_400
            } else {
              this.showErrorApp = true
